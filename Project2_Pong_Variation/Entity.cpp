    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            } else if (mVelocity.y < 0) 
            {
                ...
                // If we hit a BLOCK from below, deactivate it
...
{
    if (collidableEntities == nullptr || collisionCheckCount == 0) return;
    for (int i = 0; i < collisionCheckCount; i++)
    {
        Entity *collidableEntity = &collidableEntities[i];
...
void Entity::update(float deltaTime, Entity *collidableEntities, 
    int collisionCheckCount, Entity *secondaryEntities, int secondaryCount)
{
    ...
    checkCollisionY(secondaryEntities, secondaryCount);

    mPosition.x += mVelocity.x * deltaTime;
    checkCollisionX(collidableEntities, collisionCheckCount);

    
    checkCollisionX(secondaryEntities, secondaryCount);
