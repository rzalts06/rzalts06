...
enum AIType       { WANDERER, FOLLOWER, ORBITER, VERTICAL           };
enum AIState      { WALKING, IDLE, FOLLOWING, ORBITING, UPANDDOWN, DROWNING       };
...
private:
    ...
    void AIActivate(Entity *target, Vector2 center, float radius, float deltaTime, float orbitSpeed, float topScreen, float bottomScreen);
    void AIFollow(Entity *target);
    void AIOrbit(Vector2 center, float radius, float deltaTime, float orbitSpeed);
    void AIupAndDown(float topScreen, float bottomScreen);
